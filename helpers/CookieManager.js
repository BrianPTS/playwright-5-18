...
export class CookieManager {
  ...
  async getHeadersForEvent(eventId, proxy = null) {
    try {
      ...
      if (sessionData) {
        ...
        if (headers) {
          this.logger?.logWithTime(
            `Using session headers for event ${eventId}`,
            "debug"
          );
          return {
            headers,
      ...
      this.logger?.logWithTime(
        `Session headers failed for event ${eventId}, falling back to regular refresh`,
        "warning"
      );
      return this.refreshEventHeaders(eventId, proxy);
    } catch (error) {
      this.logger?.logWithTime(
        `Error getting session headers for event ${eventId}: ${error.message}`,
        "error"
      );
  ...
  async refreshEventHeaders(eventId, proxy = null) {
    ...
    if (
      !lastRefresh ||
      moment().diff(lastRefresh) > config.HEADER_REFRESH_INTERVAL
    ) {
      try {
  ...
  async resetCookiesAndHeaders() {
    ...
    if (
      ...
      now.diff(this.lastCookieReset) < config.COOKIE_RESET_COOLDOWN
    ) {
      this.logger?.logWithTime(
    ...
    try {
      this.logger?.logWithTime(
        "Detected multiple API failures - resetting cookies, headers and sessions",
        "warning"
      );
      ...
      try {
        ...
        this.logger?.logWithTime(
          "No cookies.json file found to delete",
          "info"
        );
      }
      ...
      this.logger?.logWithTime(
        "Applying cooldown to allow for cookie regeneration",
        "info"
      );
      ...
      this.logger?.logWithTime(
        `Error resetting cookies: ${error.message}`,
        "error"
      );
      return false;
    }
  ...
  getStats() {
    ...
    return {
      ...
      cookies: {
        ...
        lastCookieReset: this.lastCookieReset
          ? moment(this.lastCookieReset).fromNow()
          : "Never",
        capturedState: {
          hasCookies: !!this.capturedState.cookies,
          cookieCount: this.capturedState.cookies?.length || 0,
          lastRefresh: this.capturedState.lastRefresh
            ? moment(this.capturedState.lastRefresh).fromNow()
            : "Never",
        },
      },
    };
    ...
    try {
      if (fs.existsSync(this.cookiesPath)) {
        ...
        if (parsedData.cookieSets && Array.isArray(parsedData.cookieSets)) {
          const sortedSets = parsedData.cookieSets.sort(
  ...
  areCookiesFresh(cookies) {
    ...
    const authCookiesPresent = CookieManager.AUTH_COOKIES.filter(
  ...
  async saveCookiesToFile(cookies) {
    try {
      ...
      const cookieData = cookies.map((cookie) => ({
        ...
        expires:
          cookie.expires ||
          Date.now() + CookieManager.CONFIG.COOKIE_REFRESH_INTERVAL,
        expiry:
          cookie.expiry ||
          Date.now() + CookieManager.CONFIG.COOKIE_REFRESH_INTERVAL,
      }));
      ...
      if (CookieManager.CONFIG.COOKIE_ROTATION.ENABLED) {
        const shouldRotate =
          Date.now() - CookieManager.CONFIG.COOKIE_ROTATION.LAST_ROTATION >
        ...
        if (shouldRotate) {
          ...
          existingCookieSets = existingCookieSets.slice(
            0,
            CookieManager.CONFIG.COOKIE_ROTATION.MAX_STORED_COOKIES
          );
  ...
  extractEssentialCookies(cookies) {
    ...
    if (
      essentialCookies.join("; ").length <
      CookieManager.CONFIG.MAX_COOKIE_LENGTH
    ) {
  ...
  async startPeriodicCookieRefresh() {
    ...
    const scheduleNextRefresh = () => {
      ...
      console.log(
        `Next cookie refresh scheduled in ${Math.round(
          nextInterval / 60000
        )} minutes`
      );
    ...
    try {
      ...
      try {
        ...
        if (randomEvents && randomEvents.length > 0) {
          ...
          console.log(
            `Using random database event ${eventId} for cookie refresh`
          );
        } else {
        ...
        console.warn(
          `Failed to get random event from database: ${dbError.message}`
        );
      }
      ...
      if (!eventId) {
        try {
          ...
          console.log(
            `Using ScraperManager event ${eventId} for cookie refresh (database fallback)`
          );
        } catch (error) {
          console.warn(
            "Failed to get events from ScraperManager:",
            error.message
          );
          return;
        }
      ...
      if (newState?.cookies?.length) {
        console.log(
          `Successfully refreshed cookies using event ${eventId} in periodic refresh`
        );
